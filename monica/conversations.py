...
class Conversations:
    ...
    def create_conversation_object(
        self, happened_at, contact_field_type_id, contact_id
    ):
        """
    ...
    def add_message(
        self, written_at, written_by_me, content, contact_id, conversation_id
    ):
        """
        ...
        payload = json.dumps(
            payload_raw
        )  # necessary for converting Boolean to json form e.g True to true
    ...
    def add_multiple_messages(self, contact_id, conversation_id, df):
        ...
        for i in range(number_of_messages):
            written_by_me = int(
                df["written_by_me"].values[i]
            )  # 1 & 0 instead of True & False, important because numpy.bool is created from pandas
            written_at = df["date"].values[i]
            content = df["text"].values[i]
            self.add_message(
                written_at, written_by_me, content, contact_id, conversation_id
            )
