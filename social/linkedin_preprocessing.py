...
class Preprocessing:
    ...
    def prepare_dataframe(self):
        ...
        monica_contacts_df["complete_name"] = monica_contacts_df[
            "complete_name"
        ].str.lower()
        ...
        merged_df = self.combine_monica_contacts_with_linkedin_df(
            monica_contacts_df, linkedin_df_cleaned, my_name
        )

        merged_df["written_by_me"] = False
        merged_df.loc[merged_df["FROM"] == my_name, "written_by_me"] = True

        merged_df["contact_id"] = merged_df.apply(
            lambda x: max(str(x["contact_id_from"]), str(x["contact_id_to"])), axis=1
        )
    ...
    def apply(self, df):
        ...
        df.drop(
            columns=["CONVERSATION TITLE", "FOLDER", "SUBJECT", "CONTENT", "DATE"],
            inplace=True,
        )
    ...
    def prepare_fuzzy_dataframe(self, monica_contacts_df, linkedin_df_cleaned, my_name):
        ...
        to_list = list(
            linkedin_df_cleaned.drop_duplicates(subset=["TO"])["TO"].reset_index(
                drop=True
            )
        )
        from_list = list(
            linkedin_df_cleaned.drop_duplicates(subset=["FROM"])["FROM"].reset_index(
                drop=True
            )
        )
        ...
        df["monica_name"] = df["linkedin_name"].apply(
            lambda x: utils.fuzzy_contact_name_match(
                search_name=x,
                monica_contact_list=monica_contact_list,
                my_name=my_name,
                benchmark=0.8,
            )
        )
    ...
    def combine_monica_contacts_with_linkedin_df(
        self, monica_contacts_df, linkedin_df_cleaned, my_name
    ):
        utils = self.utils
        fuzzy_df = self.prepare_fuzzy_dataframe(
            monica_contacts_df, linkedin_df_cleaned, my_name
        )
        fuzzy_monica_linkedin_dict = utils.create_fuzzy_monica_linkedin_dict(fuzzy_df)

        # connect names of monica
        linkedin_df_cleaned["monica_from_full_name"] = linkedin_df_cleaned[
            "FROM"
        ].apply(
            lambda x: utils.retreive_fuzzy_monica_name(
                linkedin_name=x, fuzzy_monica_linkedin_dict=fuzzy_monica_linkedin_dict
            )
        )
        linkedin_df_cleaned["monica_to_full_name"] = linkedin_df_cleaned["TO"].apply(
            lambda x: utils.retreive_fuzzy_monica_name(
                linkedin_name=x, fuzzy_monica_linkedin_dict=fuzzy_monica_linkedin_dict
            )
        )
        ...
        linkedin_df_cleaned["contact_id_from"] = linkedin_df_cleaned[
            "monica_from_full_name"
        ].apply(
            lambda x: utils.retreive_contact_id(
                full_name=x, contact_id_dict=contact_id_dict
            )
        )
        linkedin_df_cleaned["contact_id_to"] = linkedin_df_cleaned[
            "monica_to_full_name"
        ].apply(
            lambda x: utils.retreive_contact_id(
                full_name=x, contact_id_dict=contact_id_dict
        ...
        linkedin_df_cleaned.loc[
            linkedin_df_cleaned["monica_from_full_name"] == my_name, "contact_id_from"
        ] = ""
        linkedin_df_cleaned.loc[
            linkedin_df_cleaned["monica_to_full_name"] == my_name, "contact_id_to"
        ] = ""
