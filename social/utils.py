...
class Utils:
    ...
    def create_contact_id_dict(self, monica_contacts_df):
        ...
        contact_id_dict = pd.Series(
            monica_contacts_df.id.values, index=monica_contacts_df.complete_name
        ).to_dict()
    ...
    def create_fuzzy_monica_linkedin_dict(self, fuzzy_df):
        ...
        fuzzy_monica_linkedin_dict = pd.Series(
            fuzzy_df.linkedin_name.values, index=fuzzy_df.monica_name
        ).to_dict()
    ...
    def fuzzy_contact_name_match(
        self, search_name, monica_contact_list, my_name, benchmark=0.85
    ):
        all_score = []
        if search_name != my_name:
            ...
            name_matched = self.find_max_score_name(
                monica_contact_list, all_score, benchmark
            )
            return name_matched
