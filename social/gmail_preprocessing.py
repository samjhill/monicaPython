...
class Preprocessing:
    ...
    def combine_monica_contacts_with_gmail_df(
        self, monica_contacts_df, gmail_df_cleaned, my_email
    ):
        utils = self.utils
        contact_id_dict = utils.create_contact_id_dict(monica_contacts_df)
        gmail_df_cleaned = gmail_df_cleaned.copy()

        gmail_df_cleaned["contact_id_from"] = gmail_df_cleaned["from_dict_name"].apply(
            lambda x: utils.retreive_contact_id(
                full_name=x, contact_id_dict=contact_id_dict
            )
        )
        gmail_df_cleaned["contact_id_to"] = gmail_df_cleaned["to_dict_name"].apply(
            lambda x: utils.retreive_contact_id(
                full_name=x, contact_id_dict=contact_id_dict
            )
        )

        gmail_df_cleaned.loc[
            gmail_df_cleaned["from_dict_email"] == my_email, "contact_id_from"
        ] = ""
        gmail_df_cleaned.loc[
            gmail_df_cleaned["to_dict_email"] == my_email, "contact_id_to"
        ] = ""
    ...
    def prepare_dataframe(self):
        ...
        my_email = utils.find_my_info(
            col1="to_dict_email", col2="from_dict_email", df=gmail_df_cleaned
        )
        # print(my_email)

        merged_df = self.combine_monica_contacts_with_gmail_df(
            monica_contacts_df, gmail_df_cleaned, my_email
        )
    ...
    def apply(self, df):
        ...
        df["from_dict_email"] = df["from_dict"].apply(
            lambda x: self.extract_email_address(x)
        )
        ...
        df["to_dict_email"] = df["to_dict"].apply(
            lambda x: self.extract_email_address(x)
        )
